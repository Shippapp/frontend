...
const Settings = () => {
        const [sliderValue, setSliderValue] = useState([18, 30]);
        const [distanceRange, setDistanceRange] = useState([0, 300]);
        const ethnicityOptions = [
          ...
          const [lgbtToggleValue, setLgbtToggleValue] = useState(false);
          const [pushNotificationToggleValue, setPushNotificationToggleValue] = useState(false);
        
          const handleLgbtToggleChange = (checked) => {
            setLgbtToggleValue(checked);
            // Add any other logic you need for the LGBTQIA+ toggle
          };
        
          const handlePushNotificationToggleChange = (checked) => {
            setPushNotificationToggleValue(checked);
            // Add any other logic you need for the push notification toggle
          };
  ...
  return (
    <div className="settings-container">
      <div className="top-container">
        ...
        <Link to='/shipphome' className="return-button">
          ...
          <div className="done-button">Done
          </div>
      </div>
...
{/* Middle Container*/}
    ...
    {/* Age Slider */} 
      ...
      <div>
        <div className="age-title">Age Range</div>
        <Slider
          range
          min={18}
          max={65}
          ...
          railStyle={{ backgroundColor: 'white', height: 5, width: '372px' }}
          trackStyle={[{ backgroundColor: '#E87B00', height: 5 }]}
          handleStyle={[
            { backgroundColor: 'white', borderColor: '#E87B00', height: 25, width: 25, marginLeft: 0, marginTop: -10 },
            { backgroundColor: 'white', borderColor: '#E87B00', height: 25, width: 25, marginLeft: 0, marginTop: -10 },
          ]}
        />
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
          <div className="min-value"> {sliderValue[0]}</div>
          <div className="max-value"> {sliderValue[1]}</div>
        </div>
      </div>
    </div>
    ...
    <div style={{ marginLeft: '30px', marginRight: '20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0' }}>
            <div className="min-value"> {distanceRange[0]} </div>
            <div className="max-value"> {distanceRange[1]} </div>
            </div>
      </div>

    {/* Ethnicity tab*/}
    <div style={{ marginLeft: '30px', marginRight: '20px' }}>
        <div className="ethnicity-title">Ethnicity</div>
        <Select
          defaultValue="All"
          style={{ width: '372px' }}
      ...
      <div className="setting-item">
        ...
        <div className="toggle">
          <Switch
            checked={lgbtToggleValue}
            onChange={handleLgbtToggleChange}
            style={{
              width: '65px',
              height: '33px',
              backgroundColor: lgbtToggleValue ? '#E87B00' : '#A9A9A9',
            }}
          />
        </div>
      </div>
      <div className="setting-item">
        ...
        <div className="toggle">
          <Switch
            checked={pushNotificationToggleValue}
            onChange={handlePushNotificationToggleChange}
            style={{
              width: '65px',
              height: '33px',
              backgroundColor: pushNotificationToggleValue ? '#E87B00' : '#A9A9A9',
            }}
          />
        </div>
      </div>
